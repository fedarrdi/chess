...
void undo_move(struct move *move, struct undo *undo)
{
    ...
    if(mod(eval) != mod(global_evaluation))
    {
       int a = king_move_position(&move->to);
       int b = evaluate_piece_move(&move->to);
       int c = evaluate_taking(&move->to, undo);
       int d =  center_taking(&move->to);
       int e = piece[undo->taken].weight(move->to);
       int f = space_taking(to->color);
       int g = piece_early_development(&move->to);
       int t = 1 + 2;
    }
